...
const TableInner = <DataModel,>(
  ...
  useEffect(() => {
    if (
      isPaginationEnabled &&
      !(Number.isInteger(pagination.pageSize) && Number.isInteger(totalPage))
    ) {
      throw new Error('`pageSize` and `totalPage` should be required if pagination enabled');
    }
  }, [isPaginationEnabled, pagination.pageSize, totalPage]);
  useEffect(() => {
  ...
  const handlePaginationChange: PaginationProps['onChange'] = (page, _pageSize) => {
      ...
      {isPaginationEnabled ? (
        ...
        >
          <Pagination
            ...
            total={
              Number.isInteger(pagination.pageSize) && Number.isInteger(totalPage)
                ? (pagination.pageSize as number) * (totalPage as number)
                : undefined
            }
