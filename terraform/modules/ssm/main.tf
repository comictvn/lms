...
resource "aws_ssm_parameter" "master_key" {
  name  = "/${var.name}/master_key"
  type  = "SecureString"
  value = var.master_key
}

resource "aws_ssm_parameter" "app_key" {
  name  = "/${var.name}/app_key"
  type  = "SecureString"
  value = var.app_key
}

resource "aws_ssm_parameter" "passport_private_key" {
  name  = "/${var.name}/passport_private_key"
  type  = "SecureString"
  value = var.passport_private_key
}

resource "aws_ssm_parameter" "passport_public_key" {
  name  = "/${var.name}/passport_public_key"
  type  = "SecureString"
  value = var.passport_public_key
}
